            ...
            <div class="space-y-6">
                ...
                <div class="p-4 bg-indigo-50/50 rounded-2xl border border-indigo-100/50">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                            <label class="block text-[11px] font-black text-slate-500 uppercase tracking-wider mb-2">Stok Saat Ini</label>
                            <input 
                                type="number"
                    ...
                    <div class="p-5 bg-slate-50 rounded-2xl border border-slate-200">
                        <div class="flex flex-wrap gap-4 mb-5">
                            <label class="flex items-center gap-2 cursor-pointer group">
                                <input type="radio" name="modal_mode_edit" value="manual" x-model="costPriceMode" class="w-4 h-4 text-indigo-600 focus:ring-indigo-500 border-slate-300">
                                <span class="text-sm font-bold text-slate-700 group-hover:text-indigo-600 transition-colors">Input Manual (Per Piece)</span>
                            </label>
                            <template x-if="form.satuan_grosir && form.satuan_grosir.length > 0">
                                <label class="flex items-center gap-2 cursor-pointer group">
                                    <input type="radio" name="modal_mode_edit" value="auto" x-model="costPriceMode" class="w-4 h-4 text-indigo-600 focus:ring-indigo-500 border-slate-300">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
                            ...
                            <template x-if="costPriceMode === 'auto' && getLargestUnit('edit')">
                                <div class="space-y-4">
                                    <div>
                                        <label class="block text-sm font-bold text-slate-700 mb-2" x-text="`Harga Modal per 1 ${getLargestUnit('edit').nama_satuan}`"></label>
                                        <div class="relative">
                                            <span class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 font-bold">Rp</span>
                                            <input 
                                                type="number"
                                                x-model.number="costPriceLargest"
                                                @input="calculateCostPriceFromLargest('edit', $event.target.value)"
                                        ...
                                        <p class="mt-2 text-[10px] text-slate-400 italic">
                                            * Sistem otomatis membagi harga modal <span x-text="getLargestUnit('edit').nama_satuan"></span> dengan isi (<span x-text="getLargestUnit('edit').jumlah_per_satuan"></span>) untuk mendapatkan modal per Pcs.
                                        </p>
                                    </div>
                                    <div class="p-3 bg-white rounded-xl border border-slate-200 shadow-sm">
                                        <div class="flex justify-between items-center">
                                            ...
                                            <span class="text-sm font-black text-indigo-600" x-text="'Rp ' + formatNumber(form.cost_price)"></span>
                                        </div>
                                    </div>
                                </div>
                            </template>
                            ...
                            <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm h-full flex flex-col justify-center">
                                ...
                                <div class="flex items-end justify-between">
                                    <div>
                                        <div class="text-2xl font-black" :class="(form.price - form.cost_price) > 0 ? 'text-emerald-600' : 'text-red-500'" x-text="'Rp ' + formatNumber(form.price - form.cost_price)"></div>
                                        <div class="text-[11px] font-bold mt-1" :class="(form.price - form.cost_price) > 0 ? 'text-emerald-500' : 'text-red-400'" x-text="form.price > 0 ? (((form.price - form.cost_price) / form.price * 100).toFixed(1) + '% Profit Margin') : '0% Matrix'"></div>
                                    </div>
                                    <div x-show="(form.price - form.cost_price) > 0" class="w-12 h-12 rounded-full bg-emerald-50 flex items-center justify-center text-emerald-500">
                                        <i data-lucide="trending-up" class="w-6 h-6"></i>
                                    </div>
                                    <div x-show="(form.price - form.cost_price) <= 0" class="w-12 h-12 rounded-full bg-red-50 flex items-center justify-center text-red-500">
                ...
                <div class="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-200">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-full bg-white border border-slate-200 flex items-center justify-center">
                            <i data-lucide="power" class="w-5 h-5 text-indigo-600"></i>
                        </div>
                        <div>
                            <h5 class="text-sm font-bold text-slate-900">Status Aktif</h5>
                            <p class="text-[11px] text-slate-500">Produk yang aktif akan tampil di menu kasir.</p>
                        </div>
                    </div>
